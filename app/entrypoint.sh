#!/bin/bash
# entrypoint.sh - **[Smart startup script]** (kịch bản khởi động thông minh) cho **[transformer mining container]** (container khai thác transformer)
# Tự động phát hiện và cấu hình môi trường cho hoạt động tối ưu
...
# Các biến môi trường và đường dẫn - **[eBPF variables removed]** (các biến eBPF đã bị loại bỏ)
# CSRC_DIR="/app/mining_environment/cpu_plugins/csrc/bpf"  # **[REMOVED]** (đã loại bỏ) - no eBPF
# BPF_FS_ROOT="/sys/fs/bpf"  # **[REMOVED]** (đã loại bỏ) - no eBPF
...
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# 🛡️  **[PRE-FLIGHT SANITY CHECKS]** (kiểm tra tiền điều kiện) (hostPID, memlock, perf_event_paranoid)
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
check_hostpid_and_ulimits() {
    # Generated by Copilot
    # 1) Phát hiện hostPID=false (**[PID namespace]** (không gian tên PID) riêng) – **[eBPF tracepoint]** (điểm theo dõi eBPF) không hoạt động
    # Cải thiện kiểm tra **[PID namespace]** (không gian tên PID) với nhiều phương pháp
    local hostpid_detected=false
    
    # Phương pháp 1: Kiểm tra **[inode]** (nút chỉ mục) của **[PID namespace]** (không gian tên PID)
    local init_ns="$(stat -c %i /proc/1/ns/pid 2>/dev/null || echo '')"
    local self_ns="$(stat -c %i /proc/self/ns/pid 2>/dev/null || echo '')"
    
    # Phương pháp 2: Kiểm tra xem PID 1 có phải **[systemd/init]** (trình khởi động hệ thống) không
    local pid1_comm="$(cat /proc/1/comm 2>/dev/null || echo '')"
    
    # Phương pháp 3: Kiểm tra **[mount namespace]** (không gian tên mount) (**[container]** (vùng chứa) thường có mount namespace riêng)
    local host_processes="$(ps aux | grep -v docker | grep -v containerd | wc -l 2>/dev/null || echo '0')"
    
    # Xác định hostPID dựa trên **[multiple checks]** (nhiều kiểm tra)
    if [[ -n "$init_ns" && -n "$self_ns" && "$init_ns" == "$self_ns" ]]; then
        hostpid_detected=true
        log "$LOG_INFO" "✅ Phát hiện hostPID=true qua **[PID namespace inode check]** (kiểm tra inode không gian tên PID)"
    elif [[ "$pid1_comm" == "systemd" || "$pid1_comm" == "init" ]]; then
        hostpid_detected=true
        log "$LOG_INFO" "✅ Phát hiện hostPID=true qua **[PID 1 comm check]** (kiểm tra comm của PID 1) ($pid1_comm)"
    elif [[ "$host_processes" -gt 50 ]]; then
        hostpid_detected=true
        log "$LOG_INFO" "✅ Phát hiện hostPID=true qua **[process count check]** (kiểm tra số lượng tiến trình) ($host_processes processes)"
    fi
    
    # Chỉ báo lỗi nếu tất cả **[checks]** (kiểm tra) đều thất bại
    if [[ "$hostpid_detected" == "false" ]]; then
        log "$LOG_WARN" "⚠️  Không thể xác nhận hostPID=true - **[eBPF sched_switch tracepoint]** (điểm theo dõi chuyển đổi lịch trình eBPF) có thể không hoạt động tối ưu."
        log "$LOG_WARN" "💡 Khuyến nghị: Chạy **[container]** (vùng chứa) với --pid=host hoặc đặt hostPID: true trong **[PodSpec]** (đặc tả Pod)."
        # Không exit, chỉ cảnh báo để cho phép **[container]** (vùng chứa) tiếp tục chạy
    fi

    # 2) Tự nới **[memlock]** (khóa bộ nhớ) để bpf_map_pin_to_user không thất bại
    if command -v prlimit >/dev/null 2>&1; then
        prlimit --pid $$ --memlock unlimited 2>/dev/null || \
            log "$LOG_WARN" "Không thể nới **[memlock]** (khóa bộ nhớ) – thiếu CAP_SYS_RESOURCE; hãy thêm --ulimit memlock=-1:" 
    fi

    # 3) Kiểm tra **[perf_event_paranoid]** (mức độ bảo mật sự kiện hiệu suất), disable **[IPC-sampling]** (lấy mẫu IPC) nếu **[kernel]** (nhân hệ điều hành) hạn chế
    local pep="$(cat /proc/sys/kernel/perf_event_paranoid 2>/dev/null || echo '3')"
    if [[ $pep -gt 2 ]]; then
        export DISABLE_IPC_SAMPLING=1
        log "$LOG_WARN" "perf_event_paranoid=$pep (>2) – đã đặt DISABLE_IPC_SAMPLING=1 để tắt **[IPC sampling]** (lấy mẫu IPC)."
    fi
}

# ==============================================================================
# LẤY CẤU HÌNH TỪ **[PYTHON]** (ngôn ngữ lập trình Python) ĐỂ ĐẢM BẢO TÍNH ĐỒNG BỘ
# Lấy đường dẫn PIN_DIR từ "**[single source of truth]** (nguồn chân lý duy nhất)" là ebpf_config_cpu.py.
# Điều này tránh việc phải định nghĩa đường dẫn ở cả hai nơi (**[shell]** (dòng lệnh) và **[python]** (ngôn ngữ Python)).
# ==============================================================================
# ✅ Cố định **[PYTHONPATH]** (đường dẫn Python) trước khi **[import module Python]** (nhập module Python)
export PYTHONPATH="/app:$PYTHONPATH"
# **[eBPF CPU throttling]** (điều tiết CPU eBPF) has been removed - using **[legacy throttling]** (điều tiết cũ) only

# ==============================================================================
# HÀM KHỞI TẠO **[EBPF]** (eBPF - Extended Berkeley Packet Filter) - ĐÃ LOẠI BỎ
# ==============================================================================
