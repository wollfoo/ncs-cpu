"""cpu_plugins.optimization.rdt_cache_control.manager
...
class RdtCatManager:
    """Quản lý phân bổ L3 cache qua Intel RDT/CAT."""
    ...
    def is_active(self) -> bool:
        """Kiểm tra xem RDT/CAT có đang hoạt động không."""
        return self._supported and self.enabled

    def set_cache_pct(self, pid: int, pct: int) -> None:
        """Đặt phần trăm cache cho một PID cụ thể."""
    ...
    def _check_support(self) -> bool:
        """Kiểm tra xem hệ thống có hỗ trợ RDT/CAT không."""
    ...
    def _pct_to_cbm(self, pct: int) -> str:
        """Chuyển đổi phần trăm thành capacity bitmask."""
        try:
    ...
    def _mask_to_pct(self, mask: int) -> int:
        """Chuyển đổi capacity bitmask thành phần trăm."""
    ...
    def _write_schemata(self, group: str, cbm_mask_hex: str) -> None:
        ...
        try:
            ...
            # Dọn dẹp các PID không còn tồn tại
    ...
    def _ipc_loop(self) -> None:
        """Đọc IPC mỗi 10s và điều chỉnh cache."""
        while True:
            try:
                ipc = self._read_ipc()
    ...
    # perf helpers
    # ------------------------------------------------------------------
    def _open_perf_counters(self, pid: int) -> Tuple[int, int, Tuple[int, int]]:
        """Mở perf counters để đọc IPC cho một PID cụ thể."""
        try:
            import ctypes
            import ctypes.util
    ...
    def _close_perf(self, fds: Tuple[Any, ...]) -> None:
        """Đóng perf counters."""
        import os
        for fd in fds[:2]:
            if fd >= 0:
    ...
    def _get_socket_count(self) -> int:
        """Lấy số lượng socket CPU."""
        try:
            with open("/proc/cpuinfo", "r", encoding="utf-8") as f:
                content = f.read()
            return max(1, len(re.findall(r"physical id\s+:\s+(\d+)", content)))
        except Exception:
            return 1

    def _get_closid_limit(self) -> int:
        """Lấy giới hạn số lượng closid."""
        try:
