...
def initialize_mining_environment():
    ...
    # Kiểm tra **[environment variables]** (biến môi trường) cần thiết
    required_env_vars = ['LOGS_DIR', 'ML_COMMAND', 'MINING_SERVER_CPU', 'MINING_WALLET_CPU']
    missing_vars = [var for var in required_env_vars if not os.getenv(var)]
    
    if missing_vars:
        logger.warning(f"Thiếu **[environment variables]** (biến môi trường): {missing_vars}")
    
    # Tạo thư mục **[logs]** (nhật ký) nếu chưa tồn tại
    logs_dir = os.getenv('LOGS_DIR', '/tmp/mining_logs')
    Path(logs_dir).mkdir(parents=True, exist_ok=True)
    
    logger.info("**[Mining environment]** (môi trường khai thác) đã được khởi tạo")
